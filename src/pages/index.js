        ...
        <main className={`flex min-h-screen flex-col items-center justify-between py-12 px-10 ${inter.className}`}>  
          <div className="z-10 max-w-5xl w-full font-mono text-sm">
            ...
            <div className='flex' style={{margin: '20px 0'}}>
              <button className="py-4 px-3 bg-blue-500 text-white mx-2 rounded-md" onClick={() =>  download("午饭列表.txt",stateLunchList[0].join(','))}>导出午饭列表</button>
              <button className="py-4 px-3 bg-blue-500 text-white mx-2 rounded-md" onClick={() => document.getElementById('readTxt').click()}>导入午饭列表</button>
              <input
                  style={{display: 'none'}}
                  type="file"
                  className="read-local-txt-input"
                  id="readTxt"
                  accept=".txt" onChange={e=>{
                    let selectedFile = document.getElementById('readTxt').files[0];
                    let reader = new FileReader();
                    reader.readAsText(selectedFile);
                    reader.onload = oFREvent => {
                        const pointsTxt = oFREvent.target.result;
                        const res = pointsTxt.split(',')
                        stateLunchList[1](res)
                        localStorage.setItem('eat_somethings', JSON.stringify(res));
                    };
                    e.target.value = null;
                  }}/>
        ...
        <main className={`flex min-h-screen flex-col items-center justify-between py-24 px-10 ${inter.className}`}>  
          ...
          <div className="relative flex place-items-center ">
            <div className="relative dark:drop-shadow-[0_0_0.3rem_#ffffff70] dark:invert text-center">
              ...
              <div className="text-center font-bold py-8 text-8xl" style={{wordBreak: 'break-word'}}>{stateCurrent[0]}</div>
            </div>
          </div>
          <div className="mb-32 grid text-center lg:max-w-5xl lg:w-full lg:mb-0 lg:grid-cols-4 lg:text-left"></div>
        </main>
